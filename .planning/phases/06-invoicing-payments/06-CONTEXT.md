...
- **D-61 (added post-launch, E2E-BUG-FIX-PLAN.md §6.6):** `POST /billing/payment-links` (fully built server-side) still has no mobile UI. Confirmed still wanted — flagged for a future phase to build the client, not dropped. No code change in this fix pass.
