...
export class InventoryController {
  constructor(
    private readonly service: InventoryItemService,
  ...
  getItem = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const item = await this.service.getItem(request.user.activeClinicId, params.data.itemId);
  ...
  listItems = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const result = await this.service.listItems(request.user.activeClinicId, query.data);
  ...
  getSummary = async (request: FastifyRequest, reply: FastifyReply) => {
    const summary = await this.service.getSummary(request.user.activeClinicId);
  ...
  getCategories = async (request: FastifyRequest, reply: FastifyReply) => {
    const categories = await this.service.getCategories(request.user.activeClinicId);
  ...
  getUnits = async (request: FastifyRequest, reply: FastifyReply) => {
    const units = await this.service.getUnits(request.user.activeClinicId);
  ...
  addBarcode = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const result = await this.service.addBarcode(
      request.user.activeClinicId,
      params.data.itemId,
      body.data,
    );
  ...
  removeBarcode = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    await this.service.removeBarcode(request.user.activeClinicId, params.data.barcodeId);
  ...
  receiveStock = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const result = await this.stockReceiptService.receiveStock(
      request.user.activeClinicId,
      params.data.itemId,
      request.user.id,
  ...
  getPhotoUploadUrl = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const result = await this.service.getPhotoUploadUrl(
      request.user.activeClinicId,
      params.data.itemId,
    );
  ...
  lookupBarcode = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const result = await this.barcodeLookupService.lookup(
      request.user.activeClinicId,
      query.data.code,
    );
  ...
  getBarcodeCatalog = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const result = await this.barcodeLookupService.getCatalog(
      request.user.activeClinicId,
      updatedSince,
    );
