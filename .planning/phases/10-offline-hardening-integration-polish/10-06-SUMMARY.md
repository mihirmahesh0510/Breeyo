...
**Deferred, not skipped (2026-08-26):** at the user's direction, this checkpoint is intentionally deferred until after the upcoming UI rebrand lands, since running it now against screens that are about to change would just require re-running it afterward. The backend/offline-sync work this checkpoint verifies is otherwise complete, code-reviewed, and merged -- only the physical-device drill itself waits on the rebrand.
