...
- **D-34 (amends D-26, refined 2026-08-14 during Wave 6 build):** Void restores stock only for line items added directly at billing time (e.g. Quick Sale counter items, or a manually-added invoice line whose stock movement the invoice itself created) — not for items physically administered to the patient during a consultation. A consultation-dispensed drug's stock movement stays deducted when its invoice is voided, because the physical item was consumed regardless of a billing correction; correcting a genuine mis-dispense is a separate action via Phase 5's manual "Return to stock" flow (D-51), not an automatic side effect of voiding. Where restoration does apply (billing-time items), it is unrestricted by age — D-26/D-51/D-57's 24-hour window governs only the manual per-dispense "Return to stock" action, not invoice void.
  **UI note (clarified 2026-08-14 during Wave 13 build):** because which items get restored is now fully determined by item provenance (billing-time vs. consultation-dispensed), voiding is no longer a staff "yes/no return to stock?" choice — D-26's original prompt is superseded. The void confirmation UI should be a single confirm action ("Void this invoice? Stock added at billing time will be returned automatically; items already given to the patient will not."), not a checkbox — matching `voidInvoiceSchema`'s `restoreStock: z.literal(true)` contract (no opt-out to represent).
...
- **D-39 (confirms D-27 in scope):** D-27's combined multi-invoice payment link is confirmed in scope for Phase 6, not deferred — implementation and UI design must be added to the plan before the phase closes. **API implemented (06-19d follow-up):** `POST /billing/payment-links` → `PaymentService.createCombinedPaymentLink` opens one Razorpay link for the summed outstanding balances and writes one pending `Payment` row per invoice sharing a `paymentGroupId`, which 06-10's webhook fan-out settles as a group. Groundwork was 06-03 (`paymentGroupId` + relaxed unique constraint), 06-04 (`invoiceIds: string[]`), 06-09 (`paymentGroupId` populated on every link) and 06-10 (fan-out). Covered end to end by `apps/api/tests/billing/combined-payment-link.test.ts`. The invoice-picker UI remains outstanding — see `deferred-items.md` item 14.
...
- **D-58 (added post-launch, E2E-BUG-FIX-PLAN.md §6.3):** `InvoiceActionBar`'s money actions (Collect Payment, Void, Issue Credit Note, Refund) are hidden entirely — not shown disabled — for a caller without `MANAGE_PAYMENTS`, matching how `PetInvoicesTab` and `BillingSettingsScreen` already gate elsewhere in this feature. `invoiceActionSet`/`visibleInvoiceActions` take an optional `hasManagePayments` (defaults to `true`, so every pre-existing call site keeps working); the actual gate is the new `useManagePaymentsPermission` hook, same defence-in-depth shape as `useViewInvoicesPermission`/`useBillingSettingsPermission` — the server rejects the request regardless.
- **D-59 (added post-launch, E2E-BUG-FIX-PLAN.md §6.4):** "Add Custom Service/Product" now actually adds a line — `onAddCustom` previously only closed the sheet, discarding the typed name/price. `customLineFrom(lineType, name, pricePaise, clinicDefaultRate)` in `lib/builder-screen.ts` builds a line with no `serviceCatalogId`/`inventoryItemId`, falling back to the clinic default GST rate exactly as an override-less catalog entry does.
- **D-60 (added post-launch, E2E-BUG-FIX-PLAN.md §6.4, coverage gap):** `apps/mobile` cannot render `.tsx` under the current vitest setup (`node` environment, no Metro transform, no `react-test-renderer`), which is why this exact bug — a JSX wiring mistake, not a logic bug — was invisible to the existing test suite (all `InvoiceBuilderScreen` tests exercise `lib/builder-screen.ts`, never the screen's own onPress wiring). Accepted as a standing limitation for this fix pass rather than investing in a rendering setup now; `customLineFrom` itself is covered, but the prop-wiring line (`onAddCustom={handleAddCustomService}`) is not and cannot be without that investment.
- **D-61 (added post-launch, E2E-BUG-FIX-PLAN.md §6.6):** Re-confirmed still wanted, not dropped, and deferred to a future phase — no code change in this fix pass. See `deferred-items.md` item 14 for the existing tracked gap (invoice-picker UI for `POST /billing/payment-links`).
