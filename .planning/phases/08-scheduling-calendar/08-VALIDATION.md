...
| 08-10-01 | 10 | 5 | SCH-05 | T-08-53 | Phase 7 gate verified before any producer is written; enum key checked | CLI + human | `cd apps/api && pnpm exec prisma migrate diff --from-schema-datasource "$DATABASE_URL" --to-schema-datamodel prisma/schema.prisma --exit-code` | n/a | ⬜ pending |
| 08-10-02 | 10 | 5 | SCH-05 | T-08-50 | Stale reminder tasks cancelled on reschedule; no parallel table | unit | `pnpm --filter @breeyo/api test -- --run src/modules/scheduling/__tests__/reminder.service.test.ts` | ❌ W0 | ⬜ pending |
...
| 08-11-01 | 11 | 6 | SCH-01..05 | T-08-54 / T-08-57 | clinicId only from the JWT; worker-only queries never exposed | type-check | `pnpm --filter @breeyo/api exec tsc --noEmit` | ✅ | ⬜ pending |
| 08-11-02 | 11 | 6 | SCH-01..05 | T-08-56 / T-08-59 | Permission preHandler on every route; no Worker in the routes plugin | integration | `pnpm --filter @breeyo/api test -- --run tests/queue` | ✅ | ⬜ pending |
| 08-11-03 | 11 | 6 | SCH-01..05 | T-08-55 / T-08-58 / T-08-60 | Cross-tenant returns 404 with the row untouched; range capped; room-scoped emits | integration | `pnpm --filter @breeyo/api test -- --run tests/scheduling` | ❌ W0 | ⬜ pending |
| 08-12-01 | 12 | 7 | SCH-01, SCH-03, SCH-04 | T-08-62 / T-08-63 / T-08-65 | Query keys namespaced by clinic; token-only socket handshake | unit | `pnpm --filter @breeyo/mobile test -- --run src/features/scheduling/lib/__tests__/agenda-utils.test.ts` | ❌ W0 | ⬜ pending |
| 08-12-02 | 12 | 7 | SCH-03, SCH-04 | T-08-66 | Remote updates never move the viewport or close a sheet | unit | `pnpm --filter @breeyo/mobile test -- --run src/features/scheduling` | ❌ W0 | ⬜ pending |
| 08-12-03 | 12 | 7 | SCH-01 | T-08-61 / T-08-64 | Client rules are affordances only; destructive actions confirmed and named | type-check | `pnpm --filter @breeyo/mobile exec tsc --noEmit` | ✅ | ⬜ pending |
| 08-13-01 | 13 | 7 | SCH-01 | T-08-68 | All HH:MM conversion via the single shared converter | unit | `pnpm --filter @breeyo/mobile test -- --run src/features/scheduling/lib/__tests__/availability-form.test.ts` | ❌ W0 | ⬜ pending |
| 08-13-02 | 13 | 7 | SCH-01 | T-08-67 / T-08-69 / T-08-70 / T-08-71 | Vet picker clinic-scoped; day-off with bookings requires explicit confirmation | type-check | `pnpm --filter @breeyo/mobile exec tsc --noEmit` | ✅ | ⬜ pending |
| 08-13-03 | 13 | 7 | SCH-01 | T-08-68 | Overlap and inverted-range refusals surfaced inline without losing input | unit | `pnpm --filter @breeyo/ui test` | ✅ | ⬜ pending |
| 08-14-01 | 14 | 7 | SCH-03, SCH-04 | T-08-73 / T-08-74 / T-08-76 | Token-only handshake; 401 clears session; week range always 7 days | unit | `pnpm --filter @breeyo/web test` | ❌ W0 | ⬜ pending |
| 08-14-02 | 14 | 7 | SCH-03 | T-08-72 / T-08-75 | Auth guard resolves before any data hook; no dangerouslySetInnerHTML | build | `pnpm --filter @breeyo/web exec tsc --noEmit && pnpm --filter @breeyo/web build` | ✅ | ⬜ pending |
| 08-14-03 | 14 | 7 | SCH-01, SCH-05 | T-08-77 / T-08-78 | Foreground Notification API only; no service worker, no VAPID | build | `pnpm --filter @breeyo/web exec tsc --noEmit && pnpm --filter @breeyo/web build && pnpm --filter @breeyo/web test` | ✅ | ⬜ pending |
| 08-15-01 | 15 | 8 | SCH-01..05 | T-08-83 | Requirements ticked only after every manual checkpoint is approved | full suite | `pnpm test && pnpm build` | ✅ | ⬜ pending |
| 08-15-02 | 15 | 8 | SCH-01, SCH-02 | T-08-79 | Sweep observed firing unprompted; no duplicates after a restart | human | manual — see § Manual-Only Verifications | n/a | ⬜ pending |
| 08-15-03 | 15 | 8 | SCH-03, SCH-04 | — | Non-disruptive remote updates confirmed on two live clients | human | manual — see § Manual-Only Verifications | n/a | ⬜ pending |
| 08-15-04 | 15 | 8 | SCH-05 | T-08-80 / T-08-81 / T-08-82 | Forged owner action refused; check-in survives a Redis outage; no background web push | human | manual — see § Manual-Only Verifications | n/a | ⬜ pending |
